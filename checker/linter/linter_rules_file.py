...
from checker.linter.finding_file import Finding
...
class Linter:
	...
	def _check_registers_lowercase(self):
		...
		r'\b('
		r'R[ABCD]X|R[SD]I|R[SB]P|R8|R9|R1[0-5]|' # 64 bit registers
		r'E[ABCD]X|E[SD]I|E[SB]P|'               # 32 bit registers
		r'[ABCD][XHL]|[SD]I|[SB]P'               # 16 & 8 bit registers (AX, AH, AL, etc.)
		r')\b',
		re.IGNORECASE
	)
		...
		for i, line in enumerate(self._lines, start=1):
			...
			for match in reg_pattern.finditer(line):
				register_found = match.group(1)
				
				if not register_found.islower():
					self._findings.append(Finding(
						f'Register "{register_found}" is not lowercase.',
						line_number=i,
						columns=(match.start(),),
						source=line,
					))
	...
	def _check_line_empty_with_nonzero_space(self):
		"""
		Check that empty lines have no trailing whitespace.
		"""
		for i, line in enumerate(self._lines, start=1):
			tmp_line = line.replace('\r', '').replace('\n', '')
